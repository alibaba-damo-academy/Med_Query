...
from med_query.utils.frame import Frame3d, world_to_voxel
...
class FLAREDataset(Dataset):
    def __init__(self, **kwargs) -> None:
        super(FLAREDataset, self).__init__()
        ...
        if self.aug_params.use_crop:
            ...
            for file in temp_list:
                self.image_list.extend([f"{file}-crop-{i+1}" for i in range(1)])
    ...
    def __repr__(self):
        return f"{self.dataset_name} for flare_experimentation"
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.image_dir, f"{case_name}_0000.nii.gz")
...
if __name__ == "__main__":
    ...
    cfg_module = load_module_from_file(
        os.path.join(os.path.dirname(__file__), "../configs/flare_config.py")
    )
    cfg = cfg_module.Configs()

    ds = FLAREDataset(cfg.dataset, dataset_name="trainset", phase="train")
