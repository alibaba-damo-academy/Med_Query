...
class GtGenerator(object):
    def __init__(self, **kwargs):
        ...
        self.min_voxel_thresh = kwargs.get("min_voxel_thresh", 50)

        hyper_params_json = Path(__file__).parent / "hyper_params_organ.json"
    ...
    def get_case_info(self, case, mask_rai):
        ...
        # merge raw mask
        mask_np[mask_np == 12] = 4
        mask_np[mask_np == 6] = 5
        mask_np[mask_np == 10] = 5
        mask_np[mask_np == 8] = 7
        ...
        for label_idx, label in enumerate(labels):
            ...
            if self.use_pca:
                ...
                coords = np.flipud(np.array(np.where(mask_np == label)))
                center_v = (coords.max(1) + coords.min(1)) / 2
                center_w = voxel_to_world(mask_rai, center_v).round(1)
                width, height, depth = (
                    ((coords.max(1) + 1 - coords.min(1)) * self.spacing).round(1).tolist()
                )
...
def main():
    ...
    parser.add_argument("-f", "--filelist_csv", type=str, default="/path_to/filelist.csv")
    parser.add_argument(
        "-m",
        "--mask_dir",
        type=str,
    ...
    parser.add_argument(
        "-o", "--output_dir", type=str, help="output dir", default="/path_to/output_ground_truth",
    )
    ...
    cases_info_df.to_csv(
        os.path.join(args.output_dir, "cases_info_flare_validset.csv"), index=False
    )
