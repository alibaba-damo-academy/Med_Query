...
from med_query.scripts.generate_gt_rib import GtGenerator
...
class Preprocessor(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.crop:
            random.seed(42)
            self.margin = 20  # crop_size in z-axis is larger than 2 * self.margin
            self.num_crop_for_each_sample = 3
    ...
    def resample(self, filelist, image_dir, mask_dir, spacing):
        ...
        def fun(file):
            img_path = osp.join(image_dir, f"{file}-image.nii.gz")
            msk_path = osp.join(mask_dir, f"{file}-ribmask_labelled.nii.gz")
    ...
    def dilate_mask(self, filelist, mask_dir, labels):
        ...
        Parallel(n_jobs=self.num_process)(
            delayed(fun)(file)
            for file in tqdm(filelist, desc=f"dilate {osp.basename(mask_dir)} @ ribs {labels}")
        )

    def crop_image_and_mask(self, cropped_image_dir, cropped_mask_dir):
        def fun(file):
            img_path = osp.join(self.image_dir, f"{file}-image.nii.gz")
            msk_path = osp.join(self.mask_dir, f"{file}-ribmask_labelled.nii.gz")
            img_itk = sitk.ReadImage(img_path)
            msk_itk = sitk.ReadImage(msk_path)
            ...
            assert ze - zs >= 2 * self.margin, f"FOV of {file} is too small."
            for i in range(self.num_crop_for_each_sample):
                ...
                save_img_path = osp.join(cropped_image_dir, f"{file}-crop-{i+1}-image.nii.gz")
                save_msk_path = osp.join(
                    cropped_mask_dir, f"{file}-crop-{i+1}-ribmask_labelled.nii.gz"
                )
...
def main():
    ...
    parser.add_argument(
        "-f", "--filelist_csv", type=str, default="/path/to/filelist.csv",
    )
    parser.add_argument(
        "-i",
        "--image_dir",
    ...
    parser.add_argument(
        "-m", "--mask_dir", type=str, help="dir for multi-label rib mask", default="/path/to/masks",
    )
    parser.add_argument(
        "-o", "--output_dir", type=str, help="output dir", default="/path/to/output",
    )
