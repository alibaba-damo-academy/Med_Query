...
class SingleRibDataset(Dataset):
    def __init__(self, **kwargs) -> None:
        super(SingleRibDataset, self).__init__()
        cfg = EasyDict(kwargs)
    ...
    def get_crops(self, case_name, image, mask):
        ...
        chosen = np.random.choice(bboxes, self.patches_per_volume, replace=False)
        ...
        for box in chosen:
            ...
            mask_roi_index = mask_roi_tensor == label
            if self.num_classes == 3:
                mask_roi_tensor[mask_roi_tensor > 0] = 2
            else:
                mask_roi_tensor[mask_roi_tensor > 0] = 0
            mask_roi_tensor[mask_roi_index] = 1
    ...
    def __repr__(self):
        return f"{self.dataset_name} for single rib seg"
